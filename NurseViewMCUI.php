...
include_once("NurseViewMCController.php");
include "Nurse.php";
...
$NurseMC = new NurseViewMCController();
...
<head>
    ...
    <script>
		...
		function NurseViewIndvMCFunction(medicalcertificateID) 
		{
			location.href= "NurseViewIndvMCUi.php?medicalcertificateID=" + medicalcertificateID;
		}
...
<div class="w3-content" style="max-width:1300px;margin-top:80px;margin-left:215px;overflow:hidden;">
	...
	<div class="container" style=margin-top:100px;margin-left:50px;>
        <div class="col-sm-3">
            <h4 style=margin-left:-80px;><b>Medical Certificates</b> </h4>
        </div>
	...
	<div class="table-responsive" style="height:500px;overflow:auto;width:1200px;">
		...
		if (isset($_POST['search'])) {
			$result = $NurseMC->viewPatientMC($_POST['appointmentDate']);
		} else
			$result = $NurseMC->viewPatientMC(null);
		...
		if ($result->num_rows > 0) {
			...
			while ($row = $result->fetch_assoc()) {
                //$userID = $row['userID'];
				$medicalrecordID = $row['medicalcertificateID'];
				echo "<tr><td>" . $row["medicalcertificateID"] .
					...
					style=background-color:white;color:dodgerblue;margin-top:-5px; onclick='NurseViewIndvMCFunction(".$row["medicalcertificateID"].")'>";
				echo "</td></tr>";
			}
