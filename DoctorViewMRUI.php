...
$DocMR = new DoctorViewMRController();
...
<head>
    ...
    <script>
		...
		function DoctorViewIndvMRFunction(medicalrecordID) 
		{
			location.href= "DoctorViewIndvMRUi.php?medicalrecordID=" + medicalrecordID;
		}

		function DoctorUpdateIndvMRFunction(appointmentID) 
		{
			location.href= "DoctorUpdateMRUI.php?appointmentID=" + appointmentID;
		}
...
<div class="w3-content" style="max-width:1300px;margin-top:87px;margin-left:215px;overflow:hidden;">
	...
	<div class="container" style=margin-top:100px;margin-left:50px;>
        <div class="col-sm-3">
              <h4 style=margin-left:-80px;><b>Medical Records</b>
		</div>
	...
	<div class="table-responsive" style="height:500px;overflow:auto;width:1200px;">
		...
		if (isset($_POST['search'])) {
			$result = $DocMR->viewPatientMR($_POST['appointmentDate']);
		} else
			$result = $DocMR->viewPatientMR(null);
		...
		if ($result->num_rows > 0) {
			...
			while ($row = $result->fetch_assoc()) {
				...
				echo "<tr><td>" . $row["medicalrecordID"] .
					...
					style=background-color:white;color:dodgerblue;margin-top:-5px; onclick='DoctorViewIndvMRFunction($medicalrecordID)'>".
					"<input type='button' class=btn btn-primary' value='Update' 
					style=background-color:white;color:red;margin-top:-5px; onclick='DoctorUpdateIndvMRFunction($appointmentID)'>" ;
				echo "</td></tr>";
			}
