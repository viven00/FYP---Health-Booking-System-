...
<body>
  ...
  <!-- Page content -->
  <div class="w3-content" style="max-width:1300px;margin-left:230px;overflow:hidden;">
  <div  style="margin-top:87px;max-width:1300px;background-color:#ebf5fb;">
  <!-- Automatic Slideshow Images -->
  <?php
                $viewHem = new PatientIndexController();
                $hemArr = array();
                $hemArr = $viewHem->getHem();
                if ($hemArr->num_rows > 0) {
              
               foreach ($hemArr as $row) {        
                $hemID = $row['hemID'];

                    echo "<div class=' mySlides w3-content' style='height:500px;color:#2874a6;'><div class='w3-col w3-padding-large 
                    w3-center'><img src=uploads/". $row['hemImageURL']. " style='width:580px;height:380px;float:left;padding:15px;margin-left:-120px;margin-top:50px;'>" . 
                    "<br><b><div style=margin-top:100px;margin-right:-80px;><h3>" . $row["hemTitle"] . 
                    "</b></h3></br><p style='font-size:16px;'>". $row["hemDesc"] . "</div><br><a href='ViewApptUI.php' class='btn btn-primary' style='margin-left:90px;'>Make Appointment</a>".
                    "</p></div></div></form>";
                  }
                }
                ?>
  ...
  style=font-size:34px;background-image:linear-gradient(white,#D6EAF8,white);text-align:center;max-width:1300px;font-family:GillSans; >

    <?php
    $hpm = new PatientIndexController(); 
    $result = $hpm->getHpm();
    $row = $result->fetch_assoc();
    if ($result->num_rows > 0) {
      echo "<div class='pic'>
          <div class='text'>";
      echo "<p style=font-size:40px;font-family:GillSans;>" . $row['hpmDesc'] . "</p></div></div>";
    }
    else {
      echo "<div class='pic'>
          <div class='text'>";
      echo "<p style=font-size:40px;font-family:GillSans;>No message yet</p></div></div>";
    }
    ?>   
  </div>
        ...
        <?php
               ...
               foreach ($doctorArr as $row) {        
                $userID = $row['userID'];
                    ...
                    echo "<div class='product-card' style=height:680px;width:350px;><div class='product-image'><img src=uploads/". $row['img']. " 
                    style='width:420px;height:380px;float:left;padding:15px;margin-left:-30px;'>" . 
...
<!-- Footer -->
<footer class="w3-container w3-padding-64 w3-opacity w3-light-grey w3-xlarge" style="width:1300px;">
