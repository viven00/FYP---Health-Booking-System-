...
$Patient = new ViewPatientProfileController();
...
<head>
    ...
    <script>
		function DoctorViewMCFunction(userID) 
		{
			location.href= "DoctorViewMCUI.php?userID=" + userID;
		}

		function DoctorViewMRFunction(userID) 
		{
			location.href= "DoctorViewMRUI.php?userID=" + userID;
		}

		function DoctorViewReferralFunction(userID)
		{
			location.href= "DoctorViewReferralUI.php?userID=" + userID;
		}
	</script>
</head>
<body>
		...
		if ($result->num_rows > 0) {
			...
			while ($row = $result->fetch_assoc()) {
				...
				echo "<tr><td>" . $row["userID"] .
					...
					style=background-color:white;color:dodgerblue;margin-top:-5px;margin-left:30px; onclick='DoctorViewMRFunction($userID)'>".
					"</td><td><input type='button' class=btn btn-primary' value='View' 
					style=background-color:white;color:dodgerblue;margin-top:-5px;margin-left:30px; onclick='DoctorViewMCFunction($userID)'>".
					"</td><td><input type='button' class=btn btn-primary' value='View' 
					style=background-color:white;color:dodgerblue;margin-top:-5px;margin-left:5px; onclick='DoctorViewReferralFunction($userID)'>" ;
				echo "</td></tr>";
			}
